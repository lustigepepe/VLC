...
float* fillBuffer(unsigned N, unsigned sampleSize, unsigned mono, float* temp_buffer, float* spl){
    for (unsigned i = N; i > 0; i--){
        // zero Pattern / set all not used point to 0
        if (i > sampleSize){
...
bool averageAmplitude(int n, filter_t* filter){
    ...
    for (int i = 0; i < n; i++){
        ...
        for (int j = 1; j <= length; j++){
            ...
            density += filter->p_sys->noiseDensity[i*j];
            xr += filter->p_sys->noise_xr[i*j];
            xi += filter->p_sys->noise_xi[i*j];
        }
...
frequenzStruct* fillNoiseMask(unsigned n, filter_t* filter, frequenzStruct* data, int frames){
    // if finished get out
...
frequenzStruct* denoise(int n, filter_t* filter, frequenzStruct* song){
    ...
    for (int i = 0; i < n; i++){
        ...
        if (songBuffer[i] <= noiseBuffer[i]*2000){
            song->xr[i] = 0.0f;
            song->xi[i] = 0.0f;
        }
    }
